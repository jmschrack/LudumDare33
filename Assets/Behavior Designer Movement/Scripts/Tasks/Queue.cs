...
namespace BehaviorDesigner.Runtime.Tasks.Movement
{
    ...
    public class Queue : Action
    {
        ...
        public override TaskStatus OnUpdate()
        {
            ...
            for (int i = 0; i < agents.Length; ++i) {
                if (AgentAhead(i)) {
                    agents[i].destination = agentTransforms[i].position + agentTransforms[i].forward * slowDownSpeed + DetermineSeparation(i);
                } else {
        ...
        private NavMeshAgent AgentAhead(int agentIndex)
        {
            ...
            var queueAhead = agents[agentIndex].velocity * maxQueueAheadDistance;
            for (int i = 0; i < agents.Length; ++i) {
                // Return the first agent that is ahead of the current agent
                if (agentIndex != i && Vector3.SqrMagnitude(queueAhead - agentTransforms[i].position) < maxQueueRadius) {
                    return agents[i];
                }
            }
            return null;
        ...
        private Vector3 DetermineSeparation(int agentIndex)
        {
            ...
            for (int i = 0; i < agents.Length; ++i) {
                ...
                if (agentIndex != i) {
                    ...
                    if (Vector3.SqrMagnitude(agentTransforms[i].position - agentTransform.position) < neighborDistance) {
